 ...
 * 替换某些短距平滑为长距平滑
 */
export default class JWebglProgramTypeSmoothEnumSideConnect extends JWebglProgram {
...
void main() {
    ...
    if (match (posCenterCornerForward.a, 1.0)) {
        // 通过右倾斜解决缺口问题
        if (
               match (posFLEnumBack.g, 1.0)
            && match (posRightEnumBack.g, 1.0)
        )
        {
            colorResult.g = 1.0;
            colorResult.a = 1.0;
        ...
        // 通过左倾斜解决缺口问题
        if (
                match (posFREnumBack.r, 1.0)
            && match (posLeftEnumBack.r, 1.0)
        ) 
        {
            colorResult.r = 1.0;
            colorResult.a = 1.0;
        ...
        if (
            ...
            && checkEqual (posFLColor, posLeftColor)
            && match (posLeftEnumBack.r, 1.0)
        )
        {
            colorResult.r = 1.0;
